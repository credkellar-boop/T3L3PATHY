...
class SomaticPolygraphEvaluator:
    ...
    def compute_somatic_arousal_index(self, frame_data: dict) -> float:
        ...
        hr_delta = (
            max(0.0, cardio["heart_rate_bpm"] - self.baseline_hr) / self.baseline_hr
        )
        hrv_drop = max(
            0.0, (self.baseline_hrv - cardio["hrv_rmssd_ms"]) / self.baseline_hrv
        )
        ...
        sai = (
            ...
            + (hrv_drop * 0.25)
            + (eda_spike * 0.35)
            + (resp_suppression * 0.1)
        )

        # Inject back into the frame
        frame_data["cardiovascular_physiological"]["somatic_arousal_index"] = float(
            np.clip(sai, 0.0, 1.0)
        )
